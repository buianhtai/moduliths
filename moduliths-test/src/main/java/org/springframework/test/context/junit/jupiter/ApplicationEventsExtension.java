...
/**
 * Extension for JUnit Jupiter that provides an instance of {@link ApplicationEvents} as an argument to test class
 * constructors, test methods, and test lifecycle methods.
 *
...
public class ApplicationEventsExtension implements ParameterResolver, BeforeAllCallback, AfterEachCallback {
	private static final Namespace NAMESPACE = Namespace.create(ApplicationEventsExtension.class);
	...
	/**
	 * Create an instance of {@link ApplicationEvents} and register an {@link ApplicationListener} for it in the test's
	 * {@link ApplicationContext}.
	 * <p>
	 * Note that the instance returned by this method may be cached in the {@link ExtensionContext}'s {@link Store}.
	 *
	...
	protected DefaultApplicationEvents createApplicationEvents(ParameterContext parameterContext,
		...
		return extensionContext.getStore(NAMESPACE).getOrComputeIfAbsent(DefaultApplicationEvents.class, key -> {
	...
	/**
	 * {@link ApplicationListener} that supports registration of delegate {@link ApplicationListener}s that are held in a
	 * {@link ThreadLocal} and get used in conjunction with {@link #onApplicationEvent(ApplicationEvent)} if one is
	 * registered for the current thread. This allows multiple event listeners to see the events fired in a certain thread
	 * in a concurrent execution scenario.
	 */
	private static class ThreadBoundApplicationListenerAdapter implements ApplicationListener<ApplicationEvent> {
		...
		/**
		 * Register the given {@link ApplicationListener} to be used for the current thread.
		 *
	...
	private static class DefaultApplicationEvents implements ApplicationEvents, ApplicationListener<ApplicationEvent> {
		...
		private Object unwrapPayloadEvent(Object source) {
			return PayloadApplicationEvent.class.isInstance(source) ? ((PayloadApplicationEvent<?>) source).getPayload()
					: source;
		}
